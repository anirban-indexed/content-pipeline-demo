...
Uses Claude API with Ahrefs MCP connected so Claude can do live keyword
research, SERP analysis, and competitor research exactly as it does in
the Veriheal Claude Project.
"""
...
def _load_brief_format() -> str:
    fmt_path = os.path.join(os.path.dirname(__file__), "..", "prompts", "brief_format.md")
...
{paa}"""
    ...
    return f"""You are writing a Phase 1 content brief for Veriheal.
...
H1 INSTRUCTION: The H1 must reflect the intent AND subject of the original article slug. The slug is the ground truth — if the slug says 'how-to-recover-from-edibles', the H1 must be about recovering from edibles specifically, not cannabis overconsumption generally. Do not broaden the subject beyond what the slug specifies. If the primary keyword is broader than the slug, use the slug as the H1 basis and treat the primary keyword as the SEO target only.
...
--- SLUG FIDELITY RULE — NON-NEGOTIABLE ---
...
def _write_docx(brief_text: str, url: str, output_path: str) -> None:
    doc = Document()

    # Document title
    title_para = doc.add_heading("VERIHEAL CONTENT BRIEF", level=1)
...
def generate_brief(
    ...
    Claude uses Ahrefs MCP to conduct live research during generation.
    ...
    context_text = format_context_for_prompt(context)[:12000]  # cap to ~3k tokens

    print("  Loading brief format template...")
    brief_format = _load_brief_format()
    ...
    _write_docx(brief_text, url, output_path)
