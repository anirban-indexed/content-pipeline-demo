...
- Open the article with a direct 1-2 sentence answer to the primary question, or
  a claim that establishes the article's central tension. The second sentence states
  the scope. Never open with a sentence that describes the article's structure.
  Forbidden intro patterns: "This article covers...", "In this guide, we will
  explore...", "This comprehensive guide examines...", "In this article, we'll
  walk you through...". These are AI throat-clearing patterns that signal generated
  content to both readers and search engines. The first sentence must make an
  argument or answer a question — not describe what the article is about to do.
...
- FAQ answers must be completely self-contained. Each answer must stand alone
  without any context from the article body — Google PAA and AI Overviews pull
  FAQ answers as standalone snippets. Required structure: (1) re-establish the
  subject in one phrase, (2) state the direct answer, (3) add one qualifying
  sentence. Maximum 3 sentences. Never use "as discussed above", "as mentioned
  earlier", or any back-reference to the article. Never assume the reader has
  read anything before the FAQ answer.
...
- Opening paragraphs for H2 and H3 sections must establish WHY the topic matters
  for the reader — not preview or restate the data that a list or table immediately
  below will deliver. If the opening paragraph and the structured content below it
  make the same claim, delete the prose version. Every factual claim must appear
  once. Writing it in a setup paragraph and then again in a bullet or table row
  is redundancy, not depth.
